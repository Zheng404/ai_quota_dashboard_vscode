...
| Xiaomi MiMo Token Plan | `src/services/mimo/` | Cookie (浏览器登录态) | 套餐用量统计 + 有效期展示 + 自动续费状态 |
...
│       ├── index.ts          # MiMo ServiceDescriptor 组装
│       ├── provider.ts       # MiMo 数据拉取 + 解析
│       ├── statusBar.ts      # MiMo 状态栏渲染器
│       ├── types.ts          # MimoServiceData
│       ├── template.ts       # MiMo 仪表盘卡片模板
│       ├── styles.ts         # MiMo 专属 CSS
│       └── settings.ts       # MiMo 设置表单元数据
├── storage/
...
2. **配额区域**：配额卡片垂直排列（当前套餐用量 + 补偿 Token 额度），含进度条
