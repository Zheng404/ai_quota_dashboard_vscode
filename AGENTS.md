...
| Kimi Membership | `src/services/kimi/` | JWT Token (浏览器 Cookie) | 配额进度条 + 子限额展示 + 会员等级 |
| Xiaomi MiMo Token Plan | `src/services/mimo/` | Cookie (浏览器登录态) | 套餐用量统计 + 补偿 Token 额度 + 有效期展示 + 自动续费状态 |
...
- **行为**: 进入 AFK 后，`pollAll()` 跳过数据拉取，节省资源
- **恢复**: 用户再次操作时自动恢复轮询
- **禁用**: 阈值设为 0 时关闭 AFK 检测
...
2. **配额区域**：3 个配额卡片垂直排列
   - **频限明细**：基于 `limits` 数组第一个窗口限制（通常是 5 小时频限），含子限额详情
   - **本周用量**：基于 `detail` 主配额
   - **月权益额度**：基于 `balances` 第一个余额项的 `amountUsedRatio`
