...
Cookie Bridge（独立数据流）：
浏览器扩展 → POST /cookies → CookieBridgeServer
    │
...
注意：浏览器扩展推送的凭证会自动分发到对应的 GLM/Kimi/MiMo 服务（写入 Secret Storage 并更新 `dataSource='bridge'`），无需手动配置。
```
