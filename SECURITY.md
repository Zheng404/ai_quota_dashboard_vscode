...
浏览器扩展与 VSCode 之间的凭证同步采用以下安全措施。除 Kimi / MiMo 的 Cookie 外，GLM API Key 也会通过 Bridge 转发至 VSCode：
...
- 文件名包含 VSCode 进程 PID 后缀（如 `.ai-quota-bridge-port-{pid}`），防止多实例冲突
- 当 PID 后缀文件不存在时，浏览器扩展会尝试读取通用端口文件 `.ai-quota-bridge-port`
- 若端口文件均无法读取，扩展会依次尝试回退端口范围 `[37100..37110]`
- 文件权限遵循操作系统默认临时文件策略
...
| Cookie | VSCode Secret Storage + 内存缓存 | Kimi/MiMo 服务认证 |
