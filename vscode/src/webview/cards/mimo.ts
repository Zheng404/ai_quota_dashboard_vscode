// Xiaomi MiMo Token Plan 仪表盘卡片（bundle 内置模板）
//
// 1:1 迁移自 services/mimo/template.ts 字符串版：
// - 头部三行：[名称][等级] [刷新] / [服务名] [更新时间] / [有效期至]
// - 用量统计：配额卡片垂直排列（月度Token总量/套餐Token总量）
// 函数逻辑与 DOM 输出逐行等价，仅增加类型注解与模块化

import type { MimoServiceData } from '../../services/mimo/types';
import { escapeHtml, fmtDateTime, fmtNum, registerServiceTemplate } from '../shared';

// ====== 头部渲染 ======

function renderMimoHeader(data: MimoServiceData): string {
...
function renderMimoQuota(data: MimoServiceData): string {
...
function renderMimoQuotaCard(slot: { label: string; percent: number; used?: number; limit?: number }): string {
...
/** 注册 MiMo 卡片到 bundle 内置注册表（renderService 分发） */
export function registerMimoCard(): void {
	registerServiceTemplate<MimoServiceData>('mimo', function renderCard(data) {
